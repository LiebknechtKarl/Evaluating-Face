...
detector = dlib.get_frontal_face_detector()                                         # 初始化Dlib的人脸检测器和关键点检测器
...
def normalize_landmarks(landmarks, height_range=(0, 180), width_range=(0, 140)):
    ...
    for (x, y) in landmarks:
        normalized_x = np.interp(x, [0, 1920], width_range)                         # 归一化x坐标到宽度范围
        normalized_y = np.interp(y, [0, 1080], height_range)                        # 归一化y坐标到高度范围
...
def normalize_each_face(landmarks_points, height_range=(0, 180), width_range=(0, 140)):
    ...
    for (x, y) in landmarks_points:
        normalized_x = np.interp(x, [min(np.array(landmarks_points, dtype=np.float32)[:,0]) , max(np.array(landmarks_points, dtype=np.float32)[:,0]) ], width_range)    # 归一化x坐标到宽度范围
        normalized_y = np.interp(y, [min(np.array(landmarks_points, dtype=np.float32)[:,1]) , max(np.array(landmarks_points, dtype=np.float32)[:,1]) ], height_range)   # 归一化y坐标到高度范围
        normalized_landmarks.append((normalized_x, normalized_y))
    return normalized_landmarks
...
def Space_similarity(face1,face2) :
    if len(face1) != len(face2):
        return None
    distances = []
    for p1, p2 in zip(face1, face2):                                                # # 计算每个关键点的欧几里得距离
        distance = np.linalg.norm(np.array(p1) - np.array(p2))
        distances.append(distance)
    average_distance = np.mean(distances)                                           # 计算平均距离作为空间相似性度量（距离越小，相似性越高）
    return average_distance    

def Dtw_similarity(face1_landmarks, face2_landmarks):                               # 计算每个关键点的时间相似性
    dtw_distances = []
    for i in range(68):
        seq1_x = face1_landmarks[:, i, 0].flatten().astype(np.float64)              # 提取单个关键点的 x 和 y 坐标序列
...
def Movement_smoothness(face_landmarks):
    ...
    velocity = np.diff(face_landmarks, axis=0)                                      # 计算每个点每一维的速度 shape (T-1, 68, 2)
    acceleration = np.diff(velocity, axis=0)                                        # 计算加速度 shape (T-2, 68, 2)
    acc_magnitude = np.linalg.norm(acceleration, axis=2)                            # 计算每个加速度向量的模长（L2范数） shape (T-2, 68)
    smoothness_score = np.mean(acc_magnitude)                                       # 平均加速度模长作为平滑度指标（越小越平滑）
    return smoothness_score

def process_video(input_video_path, output_matrix1_path, output_matrix2_path):
    cap = cv2.VideoCapture(input_video_path)                                        # 打开输入视频文件
    if not cap.isOpened():
        print(f"无法打开视频文件: {input_video_path}")
        return
    
    face1_landmarks_matrix = []                                                     # 初始化两个矩阵来存储人脸关键点坐标
    ...
    while cap.isOpened():
        ...
        normalized_landmarks_list = landmarks_list                                  # 归一化关键点坐标
        face1_landmarks_matrix.append(normalized_landmarks_list[0])                 # 将归一化后的关键点坐标添加到矩阵
        face2_landmarks_matrix.append(normalized_landmarks_list[1])
    cap.release()
    
    np.save(output_matrix1_path, face1_landmarks_matrix)                            # 将矩阵保存到文件
    np.save(output_matrix2_path, face2_landmarks_matrix)

    face_1 = np.array(face1_landmarks_matrix, dtype=np.float32)                     # # (208, 68, 2)  np.array(face1_landmarks_matrix, dtype=np.float32).shape
